...
{
    internal class ProblemSolver071 : ProblemSolverBase
    {
        protected override string GetSolution()
        {
            const double v = 3f / 7f;
            const int maxD = 1000000;
            var rpf = new List<Tuple<int, int>>();
            for (var d = 2; d < maxD; d++)
            {
                for (var n = 1; n < d; n++)
                {
                    if (Helper.GCD(d, n) == 1)
                    {
                        ...
                        {
                            // Shrink list
                            var data = rpf.Select(x => new { Value = x.Item1 / (double)x.Item2, Data = x }).OrderBy(x => x.Value).ToList();
                            ...
                            System.Diagnostics.Debug.Print("Shrunk! D = " + d);
            ...
            var sorted = rpf.Select(x => new { Value = (x.Item1 / (double)x.Item2), Data = x }).OrderBy(x => x.Value).ToList();
            var solution = sorted.Last(x => x.Value < v);
...
By listing the set of reduced proper fractions for d  1,000,000 in ascending order of size, find the numerator of the fraction immediately to the left of 3/7.";
        ...
        public override int ProblemNumber => 71;
    }
}
